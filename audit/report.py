...
def _format_report(session_id: str, events: list[dict]) -> str:
    ...
    warned = [e for e in tool_calls if e.get("verdict") == "WARN"]
    malformed = [e for e in tool_calls if e.get("malformed_intent")]

    lines.append("## Summary")
    lines.append("| Metric              | Count |")
    lines.append("|---------------------|-------|")
    lines.append(f"| Total tool calls    | {len(tool_calls):<5} |")
    lines.append(f"| Approved            | {len(approved):<5} |")
    lines.append(f"| Blocked             | {len(blocked):<5} |")
    lines.append(f"| Warned (below threshold) | {len(warned):<5} |")
    lines.append(f"| Ambiguous (Tier 2)  | {len(ambiguous):<5} |")
    lines.append(f"| Skipped             | {len(skipped):<5} |")
    ...
    if warned:
        lines.append("## Warnings (Below Block Threshold)")
        lines.append("")
        lines.append("These findings were logged but did not block. Raise VIGIL_MIN_BLOCK_SEVERITY to block them.")
        lines.append("")
        for e in warned:
            lines.append(f"### Call #{e.get('call_id', '?')} — WARN ({e.get('severity', 'UNKNOWN')})")
            lines.append(f"**File:** `{e.get('file', 'unknown')}`")
            lines.append(f"**Finding:** {e.get('finding', 'No details')}")
            if e.get("fix"):
                lines.append(f"**Fix:** {e['fix']}")
